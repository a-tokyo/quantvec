...
- **v128 FastScan** — a SIMD swizzle kernel (`assembly/index.ts` + `wasm/kernel.ts`): codes stored in
  a 16-vector blocked layout; per-query u8 LUT built with a global affine map so dim·max ≤ 65535 (no
  u16 overflow); `v128.swizzle` performs 16 simultaneous table lookups; u16 accumulators rank a
  candidate pool; the pool is rescored exactly via the exact WASM kernel. Enabled with `fastscan: true`
  (4-bit only; falls back to the exact kernel otherwise). Measured speedup: **1.8× on 10k vectors,
  5.7× on 50k vectors** (gain scales with n; the rescore-pool overhead is constant).
